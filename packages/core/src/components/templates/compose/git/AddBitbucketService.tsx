...
const AddBitbucketService = ({
  ...
  const { bitbucketSettings } = useWatch({
    control: form.control,
  })

  const addBitbucketNode = (data: GithubServiceType) => {
    if (type === 'update') {
      ...
      toast.success('Bitbucket details updated successfully')
    } else if (type === 'create') {
          ...
          <ScrollArea className={cn(type == 'create' ? 'h-[60vh]' : 'h-full')}>
            <div className='space-y-6'>
              <Alert variant='info'>
                ...
                <AlertDescription>
                  For now, you can set up your Bitbucket service with the
...
export default AddBitbucketService
