...
const AddGiteaService = ({
  ...
  const { giteaSettings } = useWatch({
    control: form.control,
  })

  const addGiteaNode = (data: GithubServiceType) => {
    if (type === 'update') {
      ...
      toast.success('Gitea details updated successfully')
    } else if (type === 'create') {
  ...
  return (
    <motion.div
      ...
      className='w-full'>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(addGiteaNode)}>
...
export default AddGiteaService
