...
import { ServiceNode } from '@dflow/core/components/reactflow/types'
import { Alert, AlertDescription, AlertTitle } from '@dflow/core/components/ui/alert'
import SecretContent from '@dflow/core/components/ui/blur-reveal'
import { Button } from '@dflow/core/components/ui/button'
import { DialogFooter } from '@dflow/core/components/ui/dialog'
import {
  Form,
  FormControl,
  FormField,
...
import { buildOptions } from '@dflow/core/lib/buildOptions'
...
const AddAzureDevopsService = ({
  ...
  const addAzureDevopsNode = (data: GithubServiceType) => {
    if (type === 'update') {
      ...
      toast.success('Azure Devops details updated successfully')
    } else if (type === 'create') {
  ...
  return (
    <motion.div
      ...
      className='w-full'>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(addAzureDevopsNode)}>
          <ScrollArea className={cn(type == 'create' ? 'h-[60vh]' : 'h-full')}>
            <div className='space-y-6'>
              <Alert variant='info'>
                ...
                <AlertDescription>
                  For now, you can set up your Azure DevOps service with the
                  following details. Make sure to trigger a deployment after
                  saving the changes.
                </AlertDescription>
              </Alert>
          ...
          <DialogFooter className='mt-4'>
            <Button
              type='submit'
              disabled={
                !azureSettings?.repository ||
                !azureSettings?.branch ||
                !azureSettings?.owner ||
...
export default AddAzureDevopsService
