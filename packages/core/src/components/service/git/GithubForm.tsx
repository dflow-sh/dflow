...
} from '@dflow/core/actions/gitProviders'
import { updateServiceAction } from '@dflow/core/actions/service'
import { updateServiceSchema } from '@dflow/core/actions/service/validator'
...
import { GitProvider, Service } from '@dflow/core/payload-types'
...
const GithubForm = ({
  ...
  const { execute: saveGitProviderDetails, isPending } = useAction(
    updateServiceAction,
    {
      ...
      onError: ({ error }) => {
        toast.error(
          `Failed to update Git-provider details: ${error.serverError}`,
        )
      },
    },
  )
