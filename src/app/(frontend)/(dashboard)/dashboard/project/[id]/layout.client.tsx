      ...
      {typeof server === 'object' && (
        <>
          <div
            ...
            className={cn(
              'fixed bottom-0 right-0 flex items-center justify-between border-t bg-secondary/50 px-3 py-2 transition-[width] duration-200 ease-linear hover:bg-secondary/70',
              state === 'expanded'
                ? 'w-[calc(100%-var(--sidebar-width))]'
                : 'w-[calc(100%-var(--sidebar-width-icon))]',
            )}>
            <div className='flex items-center gap-2 text-sm'>
              <SquareTerminal size={16} /> {`${server.name} - console`}
            </div>
